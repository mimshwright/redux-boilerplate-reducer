...
test("generateBoolean()", assert => {
  ...
  assertIsFunction(
    assert,
    flag.reducer,
    "generateActions.boolean().reducer() is a function."
  );
  ...
  assertIsFunction(
    assert,
    flag.setFlag,
    "Generated an action creator called setFlag()"
  );
  assert.deepEqual(
    flag.setFlag(true),
    { type: flag.SET_FLAG, payload: true },
    "Action creator for setBool works for true."
  );
  assert.deepEqual(
    flag.setFlag(false),
    { type: flag.SET_FLAG, payload: false },
    "Action creator for setBool works for false."
  );
  assert.deepEqual(
    flag.setFlag(100),
    { type: flag.SET_FLAG, payload: 100 },
    "Action creator doesn't type check."
  );

  assert.is(
    flag.TOGGLE_FLAG,
    "TOGGLE_FLAG",
    "Generated Action type called TOGGLE_FLAG"
  );
  assertIsFunction(assert, flag.toggleFlag);
  assert.deepEqual(
    flag.toggleFlag(),
    { type: flag.TOGGLE_FLAG },
    "Action creator for toggleFlag works."
  );
  ...
  assert.deepEqual(
    flag.reducer(state, action),
    true,
    "Generated reducer has a setter function"
  );
  ...
  assert.deepEqual(
    flag.reducer(state, action),
    true,
    "Generated reducer has a toggle function"
  );
});
...
test("generateNumber()", assert => {
  ...
  assert.is(
    score.SET_SCORE,
    "SET_SCORE",
    "Generated Action type called SET_SCORE"
  );
  assertIsFunction(
    assert,
    score.setScore,
    "Generated an action creator called setScore()"
  );
  assert.deepEqual(
    score.setScore(100),
    { type: score.SET_SCORE, payload: 100 },
    "Action creator for setScore works for true."
  );

  assert.is(
    score.RESET_SCORE,
    "RESET_SCORE",
    "Generated Action type called RESET_SCORE"
  );
  assertIsFunction(
    assert,
    score.resetScore,
    "Generated an action creator called resetScore()"
  );
  assert.deepEqual(
    score.resetScore(),
    { type: score.RESET_SCORE },
    "Action creator for resetScore works."
  );

  assert.is(
    score.INCREMENT_SCORE,
    "INCREMENT_SCORE",
    "Generated Action type called INCREMENT_SCORE"
  );
  assertIsFunction(
    assert,
    score.incrementScore,
    "Generated an action creator called incrementScore()"
  );
  assert.deepEqual(
    score.incrementScore(),
    { type: score.INCREMENT_SCORE },
    "Action creator for incrementScore works."
  );
...
test("generateString()", assert => {
  ...
  assertIsFunction(
    assert,
    name.setName,
    "Generated an action creator called setName()"
  );
  assertIsFunction(
    assert,
    name.resetName,
    "Generated an action creator called resetName()"
  );
...
test("extending a bundle", assert => {
  ...
  assert.is(
    score.SET_SCORE,
    "SET_SCORE",
    "addAction doesn't affect existing actions"
  );

  assert.is(
    score.DOUBLE_SCORE,
    "DOUBLE_SCORE",
    "addAction can generate an additional action type"
  );
  assertIsFunction(
    assert,
    score.doubleScore,
    "addAction can generate an additional action creator"
  );
  assert.is(
    score.reducer(150, { type: score.DOUBLE_SCORE }),
    300,
    "addAction can generate an additional reducer"
  );
  ...
  assert.is(
    score.reducer(10, { type: score.SQUARE_SCORE }),
    100,
    "addAction can generate an additional reducer"
  );
  ...
  assert.is(
    score.reducer(75, { type: score.ADD_TO_SCORE, payload: 25 }),
    100,
    "addAction can generate an additional reducer"
  );
  ...
  score.reducers.ADD_MULTIPLIER = (score, { payload: multiplier }) =>
    score * multiplier;
  ...
  assert.is(
    result,
    expected,
    "addRedcuer() can add a reducer case to an existing reducer using any arbitrary string."
  );
  ...
  assert.is(
    result,
    expected,
    "Use generate function to an existing reducer using an action not defined internally."
  );
});
