...
test('generateAction()', assert => {
  ...
  assertIsFunction(assert, action.flaggle, 'getAction() creates an alias creator function with no noun')
  ...
  assert.deepEqual(action.flaggle(123), action.flaggleMingleMoomies(123), 'alias creates the same output as main actionCreator')
})
